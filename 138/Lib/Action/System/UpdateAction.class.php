...
class UpdateAction extends BackAction{
    public function index(){
        ...
        $ver = $ver['ver'];
        $ver = substr($ver,-3);
        $updatehost = 'http://gaojishouquan.kzlai.com/update.php';
        $lastver = file_get_contents(($updatehost . '?a=check&v=') . $ver);
        if($lastver !== $ver){
            $updateinfo = ('<p class="red">最新版本为：宏宏CMS微信营销系统v ' . $lastver) . '</p><span>
		   <a href="javascript:if(confirm(\'升级前,请确认已经做好数据库和程序备份!\'))location=\'./index.php?g=System&m=Update&a=updatenow\'">点击这里在线升级</a>
            ...
            $updateinfo = ('<p class="red">最新版本为：宏宏CMS微信营销系统v ' . $lastver) . '</p><span>已经是最新系统 不需要升级</span>';
        }
