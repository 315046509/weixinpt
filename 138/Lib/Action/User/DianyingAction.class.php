...
class DianyingAction extends UserAction
{
        ...
        if (IS_POST) {
            if ($es_data == null) {
                if ($id = $data->add($_POST)) {
                    $data1['pid'] = $id;
                    $data1['module'] = 'Dianying';
                    $data1['token'] = session('token');
                    $data1['keyword'] = trim($_POST['keyword']);
                    M('Keyword')->add($data1);
                    $this->success('添加成功', U('Dianying/index', array('token' => session('token'))));
                    die;
                } else {
                ...
                if ($data->where($wh)->save($_POST)) {
                    ...
                    $this->success('修改成功', U('Dianying/index', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_add()
    {
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_son->add($_POST)) {
                    $this->success('添加成功', U('Dianying/son', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_son->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Dianying/son', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_del()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'thetype' => 'Dianying');
        $check = $t_son->where($where)->find();
        if ($check == null) {
            $this->error('操作失败');
        } else {
            ...
            if ($isok) {
                $this->success('删除成功', U('Dianying/son', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Dianying/son', array('token' => session('token'))));
            }
        }
    }
    ...
    public function housetype_add()
    {
        ...
        $son_data = M('Estate_son')->where(array('token' => session('token'),'thetype' => 'Dianying'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_housetype->add($_POST)) {
                    $this->success('添加成功', U('Dianying/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_housetype->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Dianying/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function housetype_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Dianying/housetype', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Dianying/housetype', array('token' => session('token'))));
            }
        }
    }
    ...
    public function album_add()
    {
        ...
        $check = $t_album->where(array('token' => session('token'), 'poid' => $poid,'thetype' => 'Dianying'))->find();
        $this->assign('album', $check);
        if (IS_POST) {
            if ($check == NULL) {
                $check_ex = $t_album->where(array('token' => session('token'), 'poid' => $this->_post('poid'),'thetype' => 'Dianying'))->find();
                if ($check_ex) {
                ...
                if ($t_album->add($_POST)) {
                    $this->success('添加成功', U('Dianying/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_album->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Dianying/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function album_del()
    {
        ...
        if ($check == null) {
            ...
            $isok = $housetype->where($where)->delete();
            if ($isok) {
                $this->success('删除成功', U('Dianying/album', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Dianying/album', array('token' => session('token'))));
            }
        }
    }
    ...
    public function impress()
    {
        ...
        foreach ($impress as $k => $v) {
            $son_type[] = M('Estate_impress')->where(array('id' => $v['son_id']))->field('id as sid,title')->find();
        }
        foreach ($son_type as $key => $value) {
            foreach ($value as $k => $v) {
                $impress[$key][$k] = $v;
        ...
        if (IS_POST) {
            if ($check == null) {
                if ($t_impress->add($_POST)) {
                    $this->success('添加成功', U('Dianying/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_impress->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Dianying/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function impress_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Dianying/impress', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Dianying/impress', array('token' => session('token'))));
            }
        }
    }
    public function expert()
    {
        ...
        foreach ($expert as $k => $v) {
            $son_type[] = M('Estate_expert')->where(array('id' => $v['son_id']))->field('id as sid,title')->find();
        }
        foreach ($son_type as $key => $value) {
            foreach ($value as $k => $v) {
                $expert[$key][$k] = $v;
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_expert->add($_POST)) {
                    $this->success('添加成功', U('Dianying/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_expert->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Dianying/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function expert_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Dianying/expert', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Dianying/expert', array('token' => session('token'))));
            }
        }
    }
