...
class DiningAction extends UserAction{
	...
	public function orderInfo(){
		...
		if (IS_POST){
			...
			//订餐信息
			$diningtables_model=M('dining_diningtable');
	...
	public function printOrder(){
		...
		define('MEMBER_CODE', $res['member_code']);
		define('FEYIN_KEY', $res['feyin_key']);
		define('DEVICE_NO', $res['device_no']);
		//以下2项是平台相关的设置，您不需要更改
		define('FEYIN_HOST','my.feyin.net');
		define('FEYIN_PORT', 80);

		if($this->testSendFormatedMessage()==0){
			$this->success('打印成功');
		}else{
			$this->error('打印失败');
		}
...
//----------------------以下是打印代码----------------------------
	function testSendFormatedMessage(){
		...
		*/
		$msgInfo = array (
				...
				'charge'=>'3000',  
				'customerName'=>$thisOrder['truename'],  
				'customerPhone'=>$thisOrder['tel'],  
				'customerAddress'=>$thisOrder['address'],  
				'customerMemo'=>$thisOrder['memo'],  
				'msgDetail'=>$list[0]['name']."@".$list[0]['price']."00@".$list[0]['count'],   
				'deviceNo'=>DEVICE_NO,  
				'msgNo'=>$msgNo,
		);
		// print_r($list);
	...
	function sendMessage($msgInfo) {
		import("User.Action.PrintAction");
		$client = new PrintAction(FEYIN_HOST,FEYIN_PORT);
