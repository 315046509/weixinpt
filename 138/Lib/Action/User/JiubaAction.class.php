...
class JiubaAction extends UserAction{
	...
	public function setcin(){
		$id=$this->_get('pid');
		$checkdata=$this->Yuyue_model->where(array('id'=>$id))->find();
		if(empty($checkdata)){$this->error('请填写好酒吧管理页面再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
		$cin=M('yuyue_setcin');
	...
	//订单设置
	 public function setinfo(){ 
		...
		if(empty($checkdata)){$this->error('请填写好酒吧管理页面在再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
		$_POST['token'] = $this->token;
		...
		if(empty($list[0][name])){
			$list[0][name]="选择服务套餐";
			$list[0][value]="眼部巴拿芬能量热石|香薰美白保湿护理|汉方面部推板|激光洗眉|水疗SPA";
		}
		//print_r($list);die;
		$this->assign('list',$list);
