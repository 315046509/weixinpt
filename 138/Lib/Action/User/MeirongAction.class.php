...
class MeirongAction extends UserAction{
	...
	public function setcin(){
		$id=$this->_get('pid');
		$checkdata=$this->Yuyue_model->where(array('id'=>$id))->find();
		if(empty($checkdata)){$this->error('请填写好美容管理页面再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
		$cin=M('yuyue_setcin');
	...
	//订单设置
	 public function setinfo(){ 
		...
		if(empty($checkdata)){$this->error('请填写好美容管理页面在再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
		$_POST['token'] = $this->token;
