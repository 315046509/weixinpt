...
class SchoolAction extends UserAction{
    ...
    public function del_item(){
        ...
        $this -> success('删除成功', U("School/$type", array('token' => session('token'))));
        exit;
    }
            ...
            if($t_s_score -> create() != false){
                if(session('token') == $istoken && $_id != ''){
                    ...
                    if($t_s_score -> where($where5) -> save($_POST)){
                        $this -> success('修改成功', U('School/scoresearch', array('token' => session('token'))));
                        exit;
                    }else{
                        $this -> error('服务器繁忙,修改失败,请稍候再试');
                        exit;
                    }
                }else{
                    if($id = $t_s_score -> data($_POST) -> add()){
                        $this -> success('添加成功', U('School/students', array('token' => session('token'))));
                        exit;
                    }else{
    ...
    public function introduction(){
        ...
        if(IS_POST){
            $filters = array('keyword' => array('filter' => FILTER_SANITIZE_STRIPPED, 'flags' => FILTER_SANITIZE_STRING, 'options' => FILTER_SANITIZE_ENCODED), 'title' => array('filter' => FILTER_SANITIZE_STRIPPED, 'flags' => FILTER_SANITIZE_STRING, 'options' => FILTER_SANITIZE_ENCODED), 'head_url' => array('filter' => FILTER_VALIDATE_URL), 'info' => array('filter' => FILTER_SANITIZE_STRIPPED, 'flags' => FILTER_SANITIZE_STRING, 'options' => FILTER_SANITIZE_ENCODED),);
                ...
                if(!$t_s_setindex -> create()){
                    ...
                    if('eidt' == $type && $setid != ''){
                        ...
                        if($id = $t_s_setindex -> data($_POST) -> add()){
                            ...
                            M('Keyword') -> add($data1);
                            $this -> success('添加成功', U('School/introduction', array('token' => session('token'))));
                            exit;
                        }else{
                            $this -> error('服务器繁忙,添加失败,请稍候再试');
                            exit;
            ...
            if($t_s_teachers -> create() != false){
                if(isset($_tid) && $_tid != ''){
                    ...
                    if($t_s_teachers -> where($where3) -> save($_POST)){
                        $this -> success('修改成功', U('School/assess', array('token' => session('token'))));
                        exit;
                    }else{
                        $this -> error('服务器繁忙,修改失败,请稍候再试', U('School/assess', array('token' => session('token'))));
                        exit;
                    }
                }else{
                    $_POST['createtime'] = time();
                    if($id = $t_s_teachers -> data($_POST) -> add()){
                        $this -> success('添加成功', U('School/assess', array('token' => session('token'))));
                        exit;
                    }else{
                        $this -> error('服务器繁忙,添加失败,请稍候再试', U('School/assess', array('token' => session('token'))));
                        exit;
                    }
                }
            }else{
                $this -> error($t_s_teachers -> getError());
                exit;
            }
        }
            ...
            if($t_s_students -> create() != false){
                if(isset($_id) && $_id != ''){
                    ...
                    if($t_s_students -> where($where3) -> save($_POST)){
                        $this -> success('修改成功', U('School/students', array('token' => session('token'))));
                        exit;
                    }else{
                        $this -> error('服务器繁忙,修改失败,请稍候再试');
                        exit;
                ...
                $this -> error($t_s_students -> getError());
                exit;
            }
        }
        $type = filter_var($this -> _get('type'), FILTER_SANITIZE_STRING);
            ...
            if(!$t_recipe -> create()){
                ...
                if('edit' == $status && $id != ''){
                    ...
                    if($id = $t_recipe -> data($_POST) -> add()){
                        ...
                        $data1['keyword'] = filter_var($this -> _post('keyword'), FILTER_SANITIZE_STRING);
                        M('Keyword') -> add($data1);
                        $this -> success('添加成功', U("School/cookbook", array('token' => session('token'), 'type' => $_POST['type'])));
                        exit;
                    }else{
