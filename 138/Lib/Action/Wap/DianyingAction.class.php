<?php
class DianyingAction extends BaseAction
{
    public $token;
    public $wecha_id;
    public function _initialize()
    {
        ...
        $get_ids = M('Estate')->where(array('token'=>$this->token,'thetype' => 'Dianying'))->field('res_id,classify_id')->find();
    ...
    public function index()
    {
        ...
        $this->token = $this->_get('token');
        $where       = array(
            'token' => $this->token,'thetype' => 'Dianying'
        );
    ...
    public function news()
    {
        $data        = M("Estate");
        $this->token = $this->_get('token');
        $where       = array('token' => $this->token,'thetype' => 'Dianying');
        $cid         = $data->where($where)->getField('classify_id');
        if ($cid != null) {$t_classify = M('Classify');
    ...
    public function housetype()
    {
        ...
        $where       = array('token' => $this->_get('token'),'thetype' => 'Dianying');
    ...
    public function album()
    {
        ...
        $album      = $t_album->where(array(
            'token' => $this->_get('token'),'thetype' => 'Dianying'
