...
class DiningAction extends BaseAction{
    ...
    public function __construct(){
        ...
        if($setid){
            ...
            $setinfos = $this -> company_model -> where(array('catid' => $setid)) -> find();
    ...
    public function cats(){
        ...
        $catWhere = array('parentid' => 0, 'token' => $this -> token, 'dicompanyid' => $COMPANY_ID);
        if (isset($_GET['parentid'])){
            $parentid = intval($_GET['parentid']);
            $catWhere['parentid'] = $parentid;
            $thisCat = $this -> dining_cat_model -> where(array('id' => $parentid, 'dicompanyid' => $COMPANY_ID)) -> find();
        ...
        if (isset($_GET['catid'])){
            ...
            $where['catid'] = $catid;
            $thisCat = $this -> dining_cat_model -> where(array('id' => $catid, 'dicompanyid' => $COMPANY_ID)) -> find();
            $this -> assign('thisCat', $thisCat);
            if (IS_POST){
    ...
    public function about(){
        ...
        $thisCompany = $company_model -> where(array('token' => $this -> token, 'catid' => $_GET['setid'])) -> find();
        ...
        if (IS_POST){
            ...
            if($row['payment'] == 1){
                ...
                if ($emailstatus == 1 && $emailuser){
                    ...
                    $body = "预定店铺：" . $this -> company['name'] . "<br>" . $this -> email_body() . "<a href='http://www.138wo.com' target='_blank'>【微信公众平台】</a>";
                    $this -> send_email($subject, $body, $emailuser, $emailpassword, $sys_email);
                }
    ...
    public function stores(){
        ...
        foreach ($branchStores as $k => $c){
            ...
            $branchStores[$k]['id'] = $thisCompany['id'];
