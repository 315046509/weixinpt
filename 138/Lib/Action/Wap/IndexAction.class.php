...
class IndexAction extends WapAction{
    ...
    public function index(){
        if ($this -> homeInfo['advancetpl']){
            echo '<script>window.location.href="/138mb/index.php?token=' . $this -> token . '&wecha_id=' . $this -> wecha_id . '";</script>';
            exit();
        }
        $where['token'] = $this -> token;
        ...
        include('./138/Lib/ORG/index.Tpl.php');
        foreach($tpl as $k => $v){
    ...
    public function content($contid = '', $cid = ''){
        ...
        include('./138/Lib/ORG/cont.Tpl.php');
        ...
        if ($plugmenus){
            ...
            foreach ($plugmenus as $pm){
                ...
                case 'home': if (!$pm['url']){
                    ...
                    break;
                case 'album': if (!$pm['url']){
                        $pm['url'] = '/index.php?g=Wap&m=Photo&a=index&token=' . $this -> token . '&wecha_id=' . $this -> wecha_id;
                    }
                    break;
                case 'email': $pm['url'] = 'mailto:' . $pm['url'];
                    break;
                case 'shopping': if (!$pm['url']){
                        $pm['url'] = '/index.php?g=Wap&m=Product&a=index&token=' . $this -> token . '&wecha_id=' . $this -> wecha_id;
                    }
                    break;
                case 'membercard': $card = M('member_card_create') -> where(array('token' => $this -> token, 'wecha_id' => $this -> wecha_id)) -> find();
                    if (!$pm['url']){
                        if($card == false){
                    ...
                    break;
                case 'activity': $pm['url'] = $this -> getLink($pm['url']);
                    break;
                case 'weibo': break;
                case 'tencentweibo': break;
                case 'qqzone': break;
                case 'wechat': $pm['url'] = 'weixin://addfriend/' . $this -> weixinUser['wxid'];
                    break;
                case 'music': break;
                case 'video': break;
                case 'recommend': $pm['url'] = $this -> getLink($pm['url']);
                    break;
                case 'other': $pm['url'] = $this -> getLink($pm['url']);
                    break;
                }
                $plugmenus[$i] = $pm;
                $i++;
