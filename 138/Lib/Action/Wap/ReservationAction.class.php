...
class ReservationAction extends BaseAction{
    ...
    public function index(){
        ...
        $where4 = array('token' => $token, 'wecha_id' => $wecha_id, 'rid' => $reslist['id']);
        $count = M('Reservebook') -> where($where4) -> count();
        $this -> assign('count', $count);
        $this -> display();
    }
    public function add(){
        ...
        $url .= U('Reservation/mylist', array('token' => $token, 'wecha_id' => $wecha_id, 'addtype' => $addtype));
        if($das['id'] != ''){
    ...
    public function mylist(){
        ...
        $this -> assign('wecha_id', $wecha_id);
        $book = M('Reservebook');
        $where = array('token' => $token, 'wecha_id' => $wecha_id, 'rid' => $rid);
    ...
    public function edit(){
        ...
        $reservation = M('Reservation') -> where(array('token' => $token, 'id' => $rid)) -> field('addtype,picurl,info,address,place,lng,lat,title,tel') -> find();
        if(!empty($reslist)){
        ...
        $where4 = array('token' => $token, 'wecha_id' => $wecha_id, 'rid' => $rid);
        $count = M('Reservebook') -> where($where4) -> count();
        $this -> assign('count', $count);
        $this -> display('index');
    }
