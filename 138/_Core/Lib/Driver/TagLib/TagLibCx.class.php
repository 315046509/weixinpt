...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'weiwinif'        =>  array('attr'=>'where','level'=>2),
		...
		'weiwinlist'    =>  array('attr'=>'array,foreach,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	...
	public function _weiwinif($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'weiwinif');
	...
	public function _weiwinlist($attr,$content) {
        static $_iterateParseCache = array();
        //如果已经解析过，则直接返回变量值
        $cacheIterateId = md5($attr.$content);
        if(isset($_iterateParseCache[$cacheIterateId]))
            return $_iterateParseCache[$cacheIterateId];
        $tag      = $this->parseXmlAttr($attr,'weiwinlist');
        $name   = $tag['array'];
        $id        = $tag['foreach'];
        $empty  = isset($tag['empty'])?$tag['empty']:'';
        $key     =   !empty($tag['key'])?$tag['key']:'i';
