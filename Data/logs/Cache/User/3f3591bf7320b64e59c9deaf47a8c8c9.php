...
﻿  <div class="cLineB">
    ...
    <p><span class="bold">使用方法：</span>请到每个行业模块里面的预约功能中分别添加预约</p>
				  ...
				  <tbody>
					...
					<?php if(is_array($reslist)): $i = 0; $__LIST__ = $reslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
						   ...
						   <td > 
								...
								<a href="<?php echo U('Reservation/manage',array('token'=>$vo['token'],'id'=>$vo['id']));?>" class="btn" title="订单管理">订单管理</a>

								<a class="btn" href="<?php echo U('Reservation/edit',array('token'=>$vo['token'],'id'=>$vo['id']));?>" title="编辑">编辑</a> 

								<a class="btn" href="<?php echo U('Reservation/del',array('token'=>$vo['token'],'id'=>$vo['id']));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除">删除</a></td>
