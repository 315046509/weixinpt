...
﻿<div  class="quick-actions_homepage">
  ...
  <a href='<?php echo U("Img/add");?>' title='新增图文自定义回复' class='btnGrayS vm bigbtn'><img src="<?php echo RES;?>/images/pic.png" class="vm" />新增图文自定义回复</a>　
    ...
    <select name="classid" onchange="MM_jumpMenu('parent',this,0)" style="margin-top:5px; width:auto;"  > 
                    ...
                    <option value="<?php echo U('Img/index',array('token'=>$token));?>"> 全部分类</option>
                      <?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value=" <?php echo U('Img/index1',array('classid'=>$vo['id']));?>"  <?php if($info['id'] == $vo['id']): ?>selected<?php endif; ?>> <?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>      
                    </select>
            <div class="clr"></div>
