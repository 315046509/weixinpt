...
<?php if(($parentid != '0') and ($parentid != '')): ?><a href="<?php echo U('Product/cats',array('token'=>$token,'parentid'=>$parentCat['parentid']));?>" class="btn btn-success" style="margin-top:-27px">返回上</a><?php endif; ?>
</form>
</div>
<div class="clr"></div>
</div>
...
<?php if($isDining != 1): ?><li class="tabli" id="tab2"><a href="<?php echo U('Product/shop_set',array('token'=>$token,'dining'=>0));?>">商城基本信息管理</a></li><?php endif; ?>
...
<div class="pageNavigator"> <a href="###" onclick="$('#info').submit()" title="" class="btn btn-primary"><img src="<?php echo RES;?>/images/product/arrow_switch.png" class="vm">处理订单</a></div>
<div class="clr"></div>
...
<td class="norightborder"><a href="###" onclick="showIntroDetail(<?php echo ($o["id"]); ?>)">详细</a><a href="<?php echo U('Product/printOrder',array('token'=>$token,'id'=>$o['id']));?>">打印</a><a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Product/deleteOrder',array('token'=>$token,'id'=>$o['id'],'dining'=>$isDining));?>');">删除</a></td>
