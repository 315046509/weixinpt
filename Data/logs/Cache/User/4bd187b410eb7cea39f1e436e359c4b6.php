...
﻿<div  class="quick-actions_homepage">
  ...
  <h4>上传图片</h4><a href="javascript:history.go(-1);" class="right btn btn-info" style="margin-top:-27px">返回</a>
...
<div class="pageNavigator left"><strong>此相册地址：<a href="<?php echo C('site_url');?>/index.php?g=Wap&m=Photo&a=plist&token=<?php echo ($token); ?>&id=<?php echo ($_GET['id']); ?>" target="_blank" class="green"><?php echo C('site_url');?>/index.php?g=Wap&m=Photo&a=plist&token=<?php echo ($token); ?>&id=<?php echo ($_GET['id']); ?></a></strong></div>
...
<tr>
	...
	<td class="norightborder">
		...
		<button type="submit" id="vtype" class="btn btn-info" ><strong>添加</strong></button>　
...
<tr>
	...
	<td><div class="cateimg"></div><input class="px"  style="width:250px;"   type="text"   name="picurl" value="<?php echo ($photo["picurl"]); ?>"  id="picurl<?php echo ($i); ?>"/> <script src="/tpl/static/upyun.js"></script><a href="###" onclick="upyunPicUpload('picurl<?php echo ($i); ?>',1500,1000,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('picurl<?php echo ($i); ?>')">预览</a></td>
	...
	<td class="norightborder">
		...
		<button type="submit" name="edit" value="true" class="btn btn-success vm" style="height:20px;font-size:12px;line-height:10px;">

			<strong>修改</strong>

		</button>

	<a class="btn btn-danger btn-mini" href="<?php echo U('Photo/list_del',array('id'=>$photo['id'],'token'=>session('token')));?>">删除</a>
