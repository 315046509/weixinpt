...
<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Diaoyan/add',array('token'=>$token));?>" title="添加" class="btn btn-primary"><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm">添加</a></div>
<div class="clr"></div>
...
<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
	...
	<?php
 $where = array( 'id'=> $item['id'] ); $db = M("diaoyan"); $date = $item['etime']; $arr = explode('-', $date); $date = mktime(0,0,0,$arr[1], $arr[2], $arr[0]); $now = time(); if($now>=$date){ $db-> where($where)->setField('stat',1); }else{ $db-> where($where)->setField('stat',0); } ?>
	<?php if($item['stat']==0){ ?>
		<td>进行中</td>
	<?php }elseif($item['stat']==1){ ?>
		<td>已结束</td>
	<?php } ?>
	<td class="norightborder">
		<a class="btn btn-info btn-mini" href="<?php echo U('Diaoyan/timu',array('id'=>$item['id'],'token'=>$token));?>">题目</a>&nbsp;&nbsp;
		<a class="btn btn-primary btn-mini" href="<?php echo U('Diaoyan/set',array('id'=>$item['id'],'token'=>$token));?>">修改</a>&nbsp;&nbsp;
		<a class="btn btn-success btn-mini" href="<?php echo U('Diaoyan/survey',array('id'=>$item['id'],'token'=>$token));?>">统计</a>&nbsp;&nbsp;
		<a class="btn btn-danger btn-mini" href="<?php echo U('Diaoyan/del',array('id'=>$item['id'],'token'=>$token));?>">删除</a>
	</td>
