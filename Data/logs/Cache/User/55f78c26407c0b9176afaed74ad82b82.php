...
﻿<div  class="quick-actions_homepage">
  ...
  <div class="cLineC" style="line-height: 32px;"> <a <?php if($fid==0){if($countMenu>=4){;?>
  ...
  <?php }};?>
      class="btn btn-primary">添加 <?php if($fid != 0): echo ($thisCatemenu["name"]); ?> 子<?php endif; ?>菜单</a>　 </div>
      ...
      <div id="div_ptype">
        <table class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
          ...
          <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
              ...
              <td><?php if(($vo["status"]) == "1"): ?>显示
                  <?php else: ?>
                  不显示<?php endif; ?></td>
              <td class="norightborder"><?php if ($vo['fid']==0){?>
                <a href="<?php echo U('Catemenu/index',array('fid'=>$vo['id']));?>">子菜单</a>
                <?php }?>
                <a href="<?php if ($vo['fid']==0){ echo U('Catemenu/edit',array('id'=>$vo['id'])); }else{ echo U('Catemenu/edit',array('id'=>$vo['id'],'fid'=>$vo['fid'])); }?>">编辑</a> <a href="<?php if ($vo['fid']==0){ echo U('Catemenu/del',array('id'=>$vo['id'])); }else{ echo U('Catemenu/del',array('id'=>$vo['id'],'fid'=>$vo['fid'])); }?>">删除</a></td>
