...
<li class="<?php if(ACTION_NAME == 'Reservation'): ?>current<?php endif; ?> tabli" id="tab6"><a href="<?php echo U('Reservation/lvyouindex',array('token'=>$token,'thetype'=>'estate'));?>">预定门票</a></li>
          ...
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Reservation/add',array('token'=>$token,'thetype'=>'lvyou'));?>" class="btnGrayS vm bigbtn">
              ...
              <thead>
				  ...
				  <tbody>
					<?php if(is_array($reslist)): $i = 0; $__LIST__ = $reslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
						   ...
						   <td > 
								<a href="<?php echo U('Reservation/manage',array('token'=>$vo['token'],'thetype'=>'lvyou','id'=>$vo['id']));?>" class="btn" title="订单管理">订单管理</a>
								<a class="btn" href="<?php echo U('Reservation/edit',array('token'=>$vo['token'],'thetype'=>'lvyou','id'=>$vo['id']));?>" title="编辑">编辑</a> 
								<a class="btn" href="<?php echo U('Reservation/del',array('token'=>$vo['token'],'thetype'=>'lvyou','id'=>$vo['id']));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除">删除</a></td>
