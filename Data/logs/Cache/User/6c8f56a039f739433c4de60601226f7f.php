...
<li class="<?php if(ACTION_NAME == 'Reservation'): ?>current<?php endif; ?> tabli" id="tab6"><a href="<?php echo U('Reservation/lvyouindex',array('token'=>$token,'thetype'=>'estate'));?>">预定门票</a></li>
    ...
    <p><span class="bold">使用方法：</span><?php if($album != ''): ?><span style="color:red;">（注：系统默认为至少添加3个相册,注意不要重复添加同一个相册）</span><span style="color:red;">（注：请不要重复添加同一个相册，否则删除一个同样的相册都删除）修改<strong><a href="<?php echo U('Photo/index',array('token'=>$token));?>"><font color='green'>[相册内容]</font></a></strong></span>
                                <?php else: ?>
                                 <span style="color:red;"> 请您先去［旅游区］添加景区。再来添加景点</span><?php endif; ?></p>
    </div>
</div>
          
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Lvyou/album_add',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              <thead>
                                        <tr>
                                            ...
                                            <tbody>
                                        <?php if($album == ''): ?><tr>
                                        <td colspan="5" style="text-align:center; height:30px;"><strong></strong></td>
                                        </tr>
                                    <?php else: ?>
                                    <?php if(is_array($album)): $i = 0; $__LIST__ = $album;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr> 
                                        ...
                                        <td style="">
                                        <a href="<?php echo U('Lvyou/album_add',array('poid'=>$list['id'],'token'=>$list['token']));?>" class="btn">编辑</a>
                                        <a href="<?php echo U('Lvyou/album_del',array('id'=>$list['id'],'token'=>$list['token']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn">删除</a></a>
</td>
