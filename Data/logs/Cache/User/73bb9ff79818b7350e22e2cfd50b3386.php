...
﻿  <div class="cLineB">
	...
	<div class="tab">
    <h4>订单列表 (<?php echo ($page); ?>) </h4>
		<ul>
		<li class="tabli" id="tab2"><a href="<?php echo U('Meirong/index',array('token'=>$token));?>">美容管理</a></li>
		<li class="current tabli" id="tab0"><a href="<?php echo U('Meirong/infos',array('token'=>$token,'pid'=>$pid));?>">订单管理</a></li>
		<li class="tabli" id="tab2"><a href="<?php echo U('Meirong/setcin',array('token'=>$token,'pid'=>$pid));?>">套餐设置</a></li>
		<li class="tabli" id="tab2"><a href="<?php echo U('Meirong/setinfo',array('token'=>$token,'pid'=>$pid));?>">表单设置</a></li>
		</ul>
	</div>
<a href="<?php echo U($type.'/index',array('token'=>$token));?>" class="right  btn btn-success" >返回</a>
...
<div class="msgWrap">
	...
	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><form method="post" action="" id="info">
		...
		<td><?php echo ($item["or_date"]); ?> <?php echo ($item["time"]); ?></td> 
