...
﻿<div  class="quick-actions_homepage">
    <ul class="quick-actions">
      ...
      <li class="bg_lh"> <a href="<?php echo U('Yulan/index',array('token'=>$token));?>" target="_blank"> <i class="icon-globe"></i> 在线预览</a> </li>
          ...
          <h4><?php if($tip == 1): ?>编辑幻灯片<?php else: ?>编辑背景图<?php endif; ?></h4><a href="javascript:history.go(-1);"  class="right btn btn-info" style="margin-top:-27px" >返回</a></div> 
<div class="msgWrap">
  <form class="form" method="post"   action="<?php echo U('Flash/upsave',array('id'=>$id,'tip'=>$tip));?>"  target="_top" enctype="multipart/form-data" >
...
<TR>
  ...
  <TD><input type="input" class="px" id="img" value="<?php echo ($info["img"]); ?>" name="img" style="width:500px" >  <script src="/tpl/static/upyun.js?2013"></script><a href="###" onclick="chooseFile('img','<?php if($tip == 1): ?>focus<?php else: ?>background<?php endif; ?>')" class="ChooseImage">选择</a> <a href="###" onclick="upyunPicUpload('img',<?php if($tip == 1): ?>700,420<?php else: ?>640,1136<?php endif; ?>,'<?php echo ($token); ?>')" class="ChooseImage">上传</a> <a href="###" onclick="viewImg('img')" class="ChooseImage">预览</a> 建议图片(<?php if($tip == 1): ?>宽640高320<?php else: ?>宽640高1136<?php endif; ?>)  </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="keyword">排序 ：</label></TH>
  <TD><input type="input" class="px" id="sorts" value="<?php echo ($info["sorts"]); ?>" name="sorts" style="width:100px" > <span class="red">数字越大排名越靠前</span></TD>
  <TD></TD>
</TR>
<?php if($tip == 1): ?><TR>
  <TH valign="top"><label for="keyword">幻灯片链接地址 ：</label></TH>
  <TD><input type="input" class="px" id="url" value="<?php echo ($info["url"]); ?>" name="url" style="width:300px" >  <a href="###" onclick="addLink('url',0)" class="ChooseImage">从功能库添加</a><br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<?php else: ?>
