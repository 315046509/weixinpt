  ...
  <div class="msgWrap">
    <form class="form" method="post" action="" name="modifyform"  >
      <table class="userinfoArea" style=" margin:20px 0 20px 0;" border="0" cellspacing="0" cellpadding="0" width="100%">
        <thead>
          <tr>
            <th valign="top"><label for="name">设备名称：</label></th>
            <td><input type="input" class="px" id="name" value="<?php echo ($ap["name"]); ?>" name="name" style="width:300px">
              <br></td>
          </tr>
          <tr>
            <th valign="top"> 设备位置：</th>
            <td><input name="device_location"  placeholder='' type="text" class="px" value="<?php echo ($ap["device_location"]); ?>" />
          </tr>
        </thead>
        <thead>
          <tr>
            <th valign="top">MAC：</th>
            <td><input type="input" class="px" id="mac" value="<?php echo ($ap["mac"]); ?>" name="mac" style="width:300px" /></td>
          </tr>
        </thead>
        <tbody>
          <tr>
            <th valign="top">3G网站：</th>
            <td><select id='token' name='token'>
                <?php if(is_array($webs)): $i = 0; $__LIST__ = $webs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$o): $mod = ($i % 2 );++$i;?><option value='<?php echo ($o["token"]); ?>'><?php echo ($o["wxname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
              </select>
              <a href="#myModal" data-toggle="modal" class="btn btn-success">认证地址</a></td>
          </tr>
        <div id="myModal" class="modal hide">
          ...
          <div class="modal-body">
            <p>这里选择的是3G网站用户默认连接WIFI后，自动打开的3G微站。没有经过认证前，只能访问该站…</p>
          </div>
        </div>
        <tr>
          <th valign="top"><label for="token">描述：</label></th>
          <td><textarea name="description" cols="60" rows="5"  class="px" id="description"><?php echo ($ap["mac"]); ?></textarea></td>
        </tr>
        <tr>
          <th></th>
          <td><button type="submit" name="button" class="btn btn-success left">保存</button>
            <a href="javascript:history.go(-1);" class="btn btn-info">取消</a>
...
<script>
  	<?php if($_GET['id']): ?>$('#token').val('<?php echo ($ap["token"]); ?>');
<?php else: ?>
  	$('#token').val('<?php echo (session('token')); ?>');<?php endif; ?>
