    ...
    <p><span class="bold">使用方法：</span>如果您的整体楼盘项目由多个子楼盘项目组成，请在此添加，如果没有可以不添加。</p>
          ...
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Estate/housetype_add',array('token'=>$token));?>" class="btn btn-primary">
                    <img src="tpl/User/default/common/images/pic.png" class="vm">添加子房型
                </a>
            </div>
            <div class="clr"></div>
          </div>
        ...
        <tbody>
          <?php if($housetype == ''): ?><tr>
            ...
            <?php if(is_array($housetype)): $i = 0; $__LIST__ = $housetype;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td style=""><span class="label"> <?php echo (($list['title'])?($list['title']):"独立户型"); ?></span></td>
                <td style=""><?php echo ($list['sort']); ?></td>
                <td style=""><a href="<?php echo U('Estate/housetype_add',array('id'=>$list['id'],'token'=>$list['token']));?>" class="btn">编辑</a> <a href="<?php echo U('Estate/housetype_del',array('id'=>$list['id'],'token'=>$list['token']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn">删除</a></a></td>
