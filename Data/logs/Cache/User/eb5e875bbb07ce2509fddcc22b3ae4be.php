...
查询资料[仅支持按照会员卡号查询]
	...
	<form method="post" action="">
		...
		<button type="submit" name="appidsubmit" value="true" class="btn btn-info" ><strong>查询</strong></button>
                ...
                <TR></TR>
				 ...
				 <?php if(is_array($members)): $i = 0; $__LIST__ = $members;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><TR>
                  ...
                  <TD>
                   ...
                   <?php if($list['getcardtime'] > 0): echo (date('Y-m-d H:i:s',$list["getcardtime"])); ?>
