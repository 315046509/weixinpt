    ...
    <ul class="quick-actions">
      ...
      <li class="bg_db"> <a href="javascript:history.go(-1);"> <i class="icon-share-alt"></i> 返回</a> </li>
          ...
          <div class="cLine">
            ...
            <?php if($ok == 1): ?><a href=""  title="发起文字投票" class="btn btn-primary"><img src="<?php echo RES;?>/images/text.png" class="vm">发起文字投票</a>　
<a href="" title="发起图片投票" class="btn btn-primary"><img src="<?php echo RES;?>/images/pic.png" class="vm">发起图片投票</a>
            <?php else: ?>
            <a href="<?php echo U('Vote/add',array('type'=>'text'));?>"  title="发起文字投票" class="btn btn-primary"><img src="<?php echo RES;?>/images/text.png" class="vm">发起文字投票</a>　<a href="<?php echo U('Vote/add',array('type'=>'img'));?>" title="发起图片投票" class="btn btn-primary"><img src="<?php echo RES;?>/images/pic.png" class="vm">发起图片投票</a><?php endif; ?>
  　　
              ...
              <tbody>
                <tr></tr>
                  ...
                  <td> <?php if($list['type'] == 'text'): ?>文字投票<?php else: ?>图片投票<?php endif; echo ($list["id"]); ?></td>
                   <td class="norightborder"><a class="btn btn-info btn-mini" href="<?php echo U('Vote/totals',array('type'=>$list['type'],'id'=>$list['id']));?>" title="投票结果">投票结果</a> <a class="btn btn-primary btn-mini" href="<?php echo U('Vote/edit',array('type'=>$list['type'],'id'=>$list['id']));?>" title="编辑">编辑</a> <a class="btn btn-danger btn-mini" href="javascript:drop_confirm('您确定要删除吗?删除会把投票结果也一起删除！', '<?php echo U('Vote/del',array('type'=>$list['type'],'id'=>$list['id']));?>');">删除</a> 
                   <?php if($list['status'] == 0): ?><a class="btn btn-danger btn-mini" href="javascript:drop_confirm('开始投票活动', '<?php echo U('Vote/setinc',array('type'=>$list['type'],'id'=>$list['id']));?>');">开始</a> 
                   <?php else: ?>
                   <a class="btn btn-danger btn-mini" href="javascript:drop_confirm('你是否真的想结束投票活动.', '<?php echo U('Vote/setdes',array('type'=>$list['type'],'id'=>$list['id']));?>');">结束</a><?php endif; ?>
                   </td>
