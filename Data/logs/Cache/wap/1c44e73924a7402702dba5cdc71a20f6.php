...
<div class="body" style="padding-bottom:52px;">
  ...
  <section> <a href="tel:<?php echo ($homeInfo["dianhua"]); ?>" class="link_tel icon-phone">
    <?php if($homeInfo["dianhua"] != false): echo ($homeInfo["dianhua"]); ?>
      <?php else: ?>
      没有留电话<?php endif; ?>
    </a> </section>
  <!--
  ...
  <section>
    <ul class="list_font">
      <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li style="background-color:<?php echo ($vo["yanse"]); ?>;"> <a href="<?php if($vo['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$vo ['id'],'token'=>$vo['token']));?>
          <?php else: ?>
