...
<div class="catemenu">
  <ul>
    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li> <a href="<?php if($vo['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']));?>
        ...
        "> <img style="float:right;margin-right:10px;margin-top:5px;" src="<?php echo ($vo["img"]); ?>" />
        <div style="display:block;width:40px;height:50px;float:left;margin-left:5px;margin-top:50px;" >
          <div id="aa" style="display:block;width:40px;height:50px;"><?php echo ($vo["name"]); ?></div>
        </div>
...
<?php if($showPlugMenu): ?><div class="plug-div">
    <div class="plug-phone">
      ...
      <?php if(is_array($plugmenus)): $i = 0; $__LIST__ = $plugmenus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="themeStyle plug-btn plug-btn<?php echo ($i); ?> close" style="background:<?php echo ($homeInfo["plugmenucolor"]); ?>"> <a  href="<?php echo ($vo["url"]); ?>"> <span style="background-image: url(<?php echo RES;?>/css/flash/images/img/<?php echo ($vo["name"]); ?>.png);" ></span> </a> </div><?php endforeach; endif; else: echo "" ;endif; ?>
