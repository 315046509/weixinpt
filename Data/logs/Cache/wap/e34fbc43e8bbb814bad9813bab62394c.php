...
<span class="okp">会员价：<em id="vprice">￥<?php if ($product['vprice'] > 0) {echo $product['vprice'];} else {echo $product['price'];} ?></em></span>
...
<?php if(is_array($productDetail)): $i = 0; $__LIST__ = $productDetail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pro): $mod = ($i % 2 );++$i;?><input type="hidden" id="color_<?php echo ($pro["color"]); ?>_norms_<?php echo ($pro["format"]); ?>" value="<?php echo ($pro["num"]); ?>" did="<?php echo ($pro["id"]); ?>" price="<?php echo ($pro["price"]); ?>" class="hidden"/><?php endforeach; endif; else: echo "" ;endif; ?>
<div class="i-row">
<div class="t">数量：</div>
<div class="c buynum">
...
<div class="infodetail"><?php echo ($product["intro"]); ?></div>
					...
					<!-- <div class="m-page more_detailinfo">
						<div class="pg-num"><a href="<?php echo U('Store/productDetail',array('token'=>$_GET['token'],'id'=>$product['id'],'wecha_id'=>$_GET['wecha_id']));?>">查看图文详情</a></div>
					</div>
